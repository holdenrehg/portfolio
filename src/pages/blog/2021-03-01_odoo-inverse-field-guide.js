...
const Page = props => (
    <Layout>
        <Article meta={props.meta}>
            <p>
                <span class="first-letter">T</span>he concepts of an inverse field
            ...
            <p>
                A computed field is a field on a model that automatically computes its own value based on a function instead
...
class Partner(models.Model):
    ...
    def _compute_order_count(self):
        for order in self:
            ...
            <p>
                We are inheriting the <code>res.partner</code> model and adding our own field called <code>order_count</code>.
                We marked the field computed by adding the parameter <code>compute="_compute_order_count"</code> where
            ...
            <p>
                A great way to see where and how often it's called is by adding a `_logger.info()` line to the method
                and watching the log as you click in the order forms or do different actions. I wrote an article called
                <a href="/blog/2016-06-04_odoo-it-depends" rel="noreferrer" target="_blank">Odoo — It depends</a> if you
                want to dig deeper on when and how the functions fire.
            </p>
            ...
            <p>
                You can call this "on the fly" or "just in time" computation.
            </p>
            <p>
                That obviously hurts performance depending on how you're using this field. So another
                option here is to store the field. That's a stored computed field and it has its own column in the database
                where it can persist the information for quick reads.
            </p>
...
order_count = fields.Integer(
    ...
    store=True,
            ...
            <p>
                The first step is to add <code>store=True</code> as a parameter on our field. That's what tells the ORM
                to create the column in the database. But now when does the compute function fire to update that information?
                We need to tell the system to compute whenever another field changes. In the case of <code>order_count</code>
                the order count will only change when the <code>sale_order_ids</code> field changes. A user goes in, creates
                a new sale order for partner Sally so that Sally's <code>sale_order_ids</code> relationship adds another record
                to it. At that point our compute function should fire and increment.
            </p>
            <p>
...
def _compute_order_count(self):
            ...
            <p>
                ...
                makes sense for a <strong>stored computed field</strong>. A user can't directly set the value for an on the fly
                computed field because where would their value go? There's no column in the database. There is a way for this
                to work, but I'll reference that in a special case at the end of the article.
            </p>
            <p>
...
def _set_order_count(self):
    pass
            ...
            <p>
                So we have our fancy field that both computes information and allows a user to set information. But the compute function
                itself does not care about the user data. It's going to keep doing its thing, running whenever <code>sale_order_ids</code>
                changes, and overriding whatever happens to be in the database currently, user-defined or not.
            </p>
            <p>
                The use case is a little tricky with this example. How helpful is it for a user to update the order count if it's
                going to get overwritten at times? Well there's some more we could do to it, for example we can add a flag
                that a user selects to say "I don't want this to automatically compute".
            </p>
...
def _set_order_count(self):
    pass
            ...
            <p>
                ...
                to do without it. In this case, we probably would've need a computed order_count vs a non computed order_count
                with a flag to switch back and force between them. Then maybe even another field or function to pull the correct
                information depending on the flag so that a developer down the road doesn't reference the wrong field. That could've
                been much, much more error prone.
            </p>
            ...
            <p>
                This is the part that threw me off originally when I was learning. Why do I need this empty method? What's the point of it?
            </p>
            <p>
                We need to start thinking about this like a hook. Conventionally with developers, Odoo has adopted this
                language of using <code>def _set_{'{my_field}'}</code> here so naturally we are going to think that this
                is the function that sets the information. Instead it's actually a hook. It would make more sense like
                this:
            </p>
...
def _after_order_count_set(self):
    ...
    """
            ...
            <p>
                ...
                information manually. That's why the method itself isn't actually writing to the database. In 99%
                of use cases with inverse function, we just want to let the user set their data and then that's it.
                Hence the do-nothing inverse method.
            </p>
            <p>
                There are a few situations where we might want to do some side effect, change some other field, write to
                a log, etc. after the user sets data, but generally in my experience those are rare.
            </p>
...
class Document(models.Model):
    ...
    def _after_document_set(self):
        if self.document:
            ...
            <p>
                Keep in mind I'm using a bit of pseudo code here for getting paths. But the general idea is that
                you may have a field that persists like <code>path</code> which is all you need to obtain a file's
                contents. So instead of persisting an entire binary in the database, you may just track the path and
                read the document on the fly.
            </p>
            <p>
                ...
                widget on the front end. When they edit and select their document, our <code>_after_document_set</code>
            ...
            <p>
                ...
                to think through the lifecycle of when these functions run and what they do (user enters data: inverse function fires,
                dependencies are changed: compute function fires, overwriting existing data, etc.) to see if it makes sense for your user.
            </p>
            <p>
                Inverse functions are definitely a useful tool to have handy when writing code in Odoo and I've used the feature
                a handful of times over the years. Hope this helps you all get a better grasp on the concept!
            </p>
            <p>
                Best of luck coding with it.
            </p>
        </Article>
    </Layout>
)
