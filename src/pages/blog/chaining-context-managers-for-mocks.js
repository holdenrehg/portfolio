...
const meta = new Meta({
  id: "chaining-context-managers-for-mocks",
  ...
  description: `
    ...
    solution to this.
  `,
  tags: "python,web development,testing,test mocking,refactoring",
  related: "daily-odoo-team-routine,whats-in-python3.8,simple-business-manifesto",
})
  ...
  <Layout>
    <Article meta={props.meta}>
      <p>
        ...
        ecently, I’ve used<code>unittest.mock.patch</code>to mock connections
...
def test_valid_connection_has_connected_status():
    with mock.patch("myapp.connection", MockConnection):
      ...
      <p>
        I’m going to do that by extending the<code>MockAuthenticator</code>to
        take in a parameter called<code>authenticated</code>.
      </p>
...
class MockAuthenticator:
    ...
    def __call__(self, *args, **kwargs):
      ...
      <p>
        Then we can pull out some variables for each scenario to re-use
        throughout the entire test suite. This actually isn’t a bad option.
      </p>
...
def test_invalid_connection_has_disconnected_status():
    with test_connection, unauthed:
      ...
      <p>
        ...
        <code>test_connection</code>on every test, no matter what type of
        connection I was using. I attempted to abstract those up into a single
        context manager, but ran into issues.
      </p>
      <p>
        The<code>with</code>statement can handle multiple “arguments” as you
        see in the example above, but you can’t pass an iterable or unpack an
        iterable the way you can with function calls. That would make things
...
def test_valid_connection_has_connected_status():
    ...
    with authed():
      ...
      <p>
        The best option in my mind is to pull out two functions that make it
        clear what is going on in each test, while mocking multiple objects
        behind the scenes.
      </p>
      <p>
        We can do this by defining<code>contextmanager</code>functions, running
        each mock as needed and then<code>yield</code>ing to the calling
        function.
        <a
        ...
        <code>__enter__</code>and<code>__exit__</code>methods like you would in
        a class definition.
      </p>

      <p>
        Now we redefine our tests. Check them out. I personally think they are
        much easier to understand at a glance, with a simple interface for auth
        mocks. Plus it will save us a few characters as an added bonus.
      </p>
...
def test_invalid_connection_has_disconnected_status():
    with unauthed():
      ...
      <p>
        While it doesn’t make sense for the exact scenario we’re dealing with
        above, we could have also potentially used an<code>ExitStack</code>.
      </p>

      <p>
        I had no clue what an<code>ExitStack</code>was until I started hunting
        around for a way to chain together context managers dynamically. In the
        <a
        ...
        it’s defined as “[a way] to make it easy to programmatically combine
        other context managers and cleanup functions, especially those that are
        optional or otherwise driven by input data.”
      </p>

      <p>
        Sounds great. The general idea behind them is that I can enter into
        the<code>ExitStack</code>and have more control over the context managers
        that I push onto the stack. When the<code>ExitStack</code>closes then it
        iteratively pops every registered context manager off the stack and
        closes them in reverse order of their registration (first in last out).
      </p>
...
with open("file_1.txt"):
    with open("file_2.txt"):
      ...
      <p>
        The primary benefit is that while you cannot, for example, iterate over
        a list of files and pass a list of<code>open</code>results into
        a<code>with</code>statement, we can do that with the
        <code>ExitStack</code>.
      </p>

      <p>
        We can take a look at another common example when dealing with mocks.
        Assume that you need to test a connection to an external API or database
        and you want to create a single test that interacts with a number of
        outside mocks.
      </p>
...
def test_sqlite_database_adapters():
    with mock.patch("db.connection", MockSQLiteConnection):
      ...
      <p>
        Instead, we can create a context manager that iterates over all possible
        adapters, creates a new context for them via a
        <code>db.connect</code>function, and then yields those back to the test.
        Once that test has finished running, then the<code>ExitStack</code>handles
        all of the cleanup for those connections.
      </p>
...
def test_database_adapters():
    with db_env(SUPPORTED_ADAPTERS) as connections:
      ...
      <p>
        Context managers can be an extremely useful abstraction in your python
        code if you don’t go overboard with them. Give it a shot in your code or
        tests to see where that might make sense.
      </p>
    </Article>
  </Layout>
)
