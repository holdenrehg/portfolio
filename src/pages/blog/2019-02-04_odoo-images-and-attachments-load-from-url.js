...
const Page = props => (
  <Layout>
    <Article meta={props.meta}>
      <p><span className="first-letter">T</span>his is the first in a series of posts about images and attachments in Odoo. I think it’s a topic that not all Odoo developers are completely clear on.</p>
...
class ProductTemplate(models.Model):
    ...
    image = fields.Binary(
      ...
      <p>In the example above we are looking at a <code>Binary</code> field called <code>image</code> which is stored on the <code>product.template</code> model, not as an attachment. The default for <code>Binary</code> fields will be <code>attachment=False</code> .</p>
      <p>So if you take a look at the database table, in this case, it would be <code>product_template</code> we will see a blob column for the<code>image</code> field. The contents contain a <strong>base64 encoded binary string</strong> of the image.</p>
...
class StoresImages():
    ...
    def fetch_image_from_url(self, url):
      ...
      <p>In this mixin, we have a basic <code>fetch_image_from_url</code> function which will take in a URL pointing to an image and then return the proper string which we can store in the database.</p>

      <h3>Then Add Your Helper To Data Models</h3>
      <p>Use your mixin on your data models where you need access to functions like the <code>fetch_image_from_url</code> function we created above.</p>
      <code className="python">
        <pre>{`
class CustomModel(models.Model, StoresImages):
    ...
    def _compute_image(self):
      ...
      <p>Here we created our own custom model called <code>my.custom.model</code> . You can see that it uses our <code>StoresImages</code> mixin:</p>
      <code className="python">
        <pre>{`
class CustomModel(models.Model, StoresImages)
        `}</pre>
...
image_url = fields.Char(
    ...
    required=True
      ...
      <p>Instead of overriding the<code>write</code> function or updating the image with Binary strings to actually display the image, we can create a simple <code>compute</code> field that automatically converts the <code>image_url</code> field when it changes. This compute function can use our mixin/helper to make the code clean:</p>
      <code className="python">
        <pre>{`
image = fields.Binary(
    string="Image",
