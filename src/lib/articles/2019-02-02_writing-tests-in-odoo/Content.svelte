<h2>Introduction</h2>
<p>
    <span class="first-letter">T</span>esting with Odoo, in any version, is not 100% straight forward. There
    are some different tricks and requirements that aren’t always made clear.
</p>
<p>
    So I wanted to write up everything that my team and I at <a
        href="https://bluestingray.com"
        rel="noreferrer"
        target="_blank">Blue Stingray</a
    > have learned about Odoo testing over the last few years. We’ve luckily been able to learn a ton.
</p>
<p>
    This is not an explanation on <strong>what</strong> to test. There are plenty of great articles and books out
    there for what should you be testing, and maybe I’ll write up a summary of those one day. But for now, this
    is strictly an explanation on how to organize, write, and run some example tests in Odoo.
</p>
...
<p>
    I will be referencing Odoo 12, but Odoo 9 to 12 have pretty similar testing frameworks. There were no huge
    changes between recent versions.
</p>
<p>
    Also, you may want to check out a <a href="/blog/2019-01-27_dockerized-odoo-development"
        >Simple Introduction To Docker Development In Odoo</a
    > since I will be referencing some Docker commands.
</p>

<h2>Module Structure</h2>
<p>
    Let’s create a sample module. For reference, all sample code is in a repository at
    <a href="https://github.com/holdenrehg/sample_test_module" rel="noreferrer" target="_blank"
        >https://github.com/holdenrehg/sample_test_module</a
    >.
</p>
<code class="console">
    <pre>{`
...
<p>
    ...
    types of tests. In the example above you can see that we have <code>tests/unit</code>
    for all of our unit tests. Maybe if we kept building this out, we would introduce
...
<p>
    Again, this is just convention to have a <code>models</code> folder and a <code>helpers</code>
    folder for Odoo modules, but as long as your <code>__init__</code> files are properly importing files you can
    organize your module however you would like.
</p>
<code class="console">
    <pre>{`
test_module/helpers
...
<h2>Module Files</h2>
<p>
    Let’s dig into the contents of some of these sample files that I’ve written for you. I’m not quite going
    to get into the <code>tests</code> folder until the section below.
</p>
<h3>__manifest__.py</h3>
<p>
    Because we are using 12.0 we require a manifest file. If we were looking at 9.0 and back we may have an <code
        >__openerp__.py</code
    > file in the root of our module, but the general contents will be the same.
</p>
<code class="python">
    <pre>{`
# -*- coding: utf-8 -*-
...
<p>
    We have an extremely creative, and definitely unique (no one has used Twitter for their development
    tutorial before, right?) set of models to mess around with.
</p>
<p>
    The first is a Twitter settings model. This is going to be a class that stores a username and a password.
    These would be credentials for accessing Twitter accounts if this module was expanding out to a
    functioning module.
</p>
<code class="python">
    <pre>{`
...
<p>
    The second is a Tweet model. This is a data model that contains the description of the tweet, and
    constraints for the tweet. For example, you cannot generate a tweet that is longer than 140 character.
</p>
<p>
    This gives us a clear restriction where we can write some tests to ensure that restriction is being
...
<p>
    Now we need 2 helpers. We have the Twitter data models, but we need an easy way to make a tweet and an
    easy way to truncate off characters down.
</p>
<p>
...
<p>
    How do we write a test for it? First, there are some requirements specific to the Odoo testing framework
    that we must adhere to. You’ll see how these apply in the sample tests I’ve written, but here’s what you
    need to think about:
</p>
<ul>
...
<p>
    Make sure to always include your test file in this <code>__init__.py</code> file for the system to recognize
    the test.
</p>
...
<p>
    You can get a little bit creative with your module organization, except when it comes to things like
    tests, make sure they are in the <code>tests</code> folder.
</p>
...
<p>
    As you are making your tests (and putting them in the <code>tests</code> folder), make sure that all of
    your test files are prefixed with <code>test_</code> .
</p>
<p>
    There’s a reason why, in my example, that my test files are named things like <code
        >test_tweeter_helper.py</code
    >
    or <code>test_string_helper.py</code> .
</p>
<h3>All Test Methods Must Start With test_</h3>
<p>
    I haven’t actually shown you a full test class or method yet, but when you add methods to your test class
    (which is inside of the <code>tests</code> folder and inside of a file prepended with
    <code>tests_&lt;filename&gt;.py</code>) then you also have to prefix it with <code>test_</code> .
</p>
...
<p>
    This is our unit test for the string helper. So this is under <code>tests/unit/test_string_helper.py</code
    >
    and is testing our helper at <code>helpers/string.py</code> .
</p>
<p>
    I’m not going to dig too far into what you should be trying to test for or all of the assertion method
    available to you (check out the <a
        href="https://docs.python.org/2/library/unittest.html#assert-methods"
        rel="noreferrer"
        target="_blank">list of assertion methods for unittest2</a
    >
    or the <code>odoo/test/common.py</code> file in Odoo core.)
</p>
...
<p>
    To do this, I need to import our string helper from the module via a <code
        >from odoo.addons.test_module.helpers import string</code
    >
    call, create some sample strings to work with, call the <code>string.limit(...)</code> function that I’m trying
    to test, and then assert that the return values are equal to the expected results.
</p>
<code class="python">
    <pre>{`
def test_string_should_do_nothing_when_same_size(self):
...
<a
    href="https://github.com/holdenrehg/sample_test_module/tree/master/test_module/tests/unit"
    rel="noreferrer"
    target="_blank"
/>
...
<h2>Running Tests</h2>
<p>
    Finally, we can try to run some tests. I’m going to show you how to run these via Docker and Docker
    Compose, but you can also run these directly from the <code>odoo.py</code> or
    <code>odoo-bin</code> executables.
</p>
<h3>Requirements</h3>
<p>More requirements.</p>
<p>
    Similar to the requirements for writing a test. There are a few things that we need for running the test
    suite.
</p>
<ul>
    <li>
        A database instance WITH demo data. So when you are creating a database from that database creation
        screen, <strong>make sure to mark to include demo data</strong>.
    </li>
...
<p>
    2. Start up the instance and create a new database called <code>test_1</code>
    <strong>with demo data</strong>. If you do not make your database first, the command in step #3 will
    automatically create the database but also run tests on core modules (this can take a while).
</p>
<code class="console">
    <pre>{`
$ docker-compose up -d
...
<p>
    2. Make a database called <code>test_1</code> <strong>with demo data</strong>. If you do not make your
    database first, the command in step #3 will automatically create the database but also run tests on core
    modules (this can take a while).
</p>
...
<img src="/images/articles/writing-tests-in-odoo/good-results.png" />
<p>
    It gives you a lot of information to parse, but you will be looking for each section that says something
    similar to<code>odoo.addons.test_module.tests.unit.test_string_helper running test</code> because that will
    be an individual test class/test case. You can see where it runs each method in the test case.
</p>
...
<img src="/images/articles/writing-tests-in-odoo/good-results.png" />
<p>
    You’ll see where again we get the logger for <code
        >odoo.addons.test_module.tests.unit.test_string_helper running tests</code
    > and then each method that runs below it.
</p>
...
<p>
    Pretty clear that our <code>string.limit</code> method ran with <code>size=6</code> but we were expecting the
    length of the final string to be equal to 5. This looks like a problem with our test instead of the functionality
    of the method, so we can fix that and re-run our test suite.
</p>

<h2>Conclusion</h2>
<p>
    There’s a little bit of an additional learning curve testing in Odoo, especially if you are used to
    working on a vanilla python project with tools like pytest or unittest2.
</p>
<p>But if you are an Odoo developer day-to-day it’s worth it.</p>
<p>Write more tests.</p>
<p>Then write even more tests.</p>
<p>
    They can save your ass in the long run especially as your testing suite builds up and provides more and
    more coverage of your project.
</p>
