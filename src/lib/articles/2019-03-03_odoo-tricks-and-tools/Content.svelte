<h2>Intro</h2>
<p>
    <span class="first-letter">T</span>here’s a set of built-in <code>tools</code> associated with the Odoo
    framework that many developers / third-party modules often overlook (found in the Odoo core under
    <code>odoo/tools</code>).
</p>
<p>I’m going to outline and provide examples for some of the most common functions and variables.</p>

<h2>Preface</h2>
<p>
    All of the commands run below, I’m running in an Odoo shell instance. It makes it easy to test out all of
    the different tools available to you. If you aren’t sure how to run an Odoo shell, then <a
        href="/blog/2019-02-16_run-an-odoo-repl">see my article on it called Run An Odoo REPL</a
    >.
</p>
...
<h3>config</h3>
<p>
    Some helpers related to the odoo.conf configuration file or the configuration parameters passed into the
    odoo.py executable at start.
</p>
<code class="python">
    <pre>{`
...
<p>
    Conversion helpers. These are primarily focused on conversion of data types such as xml and csv specific
    to Odoo, but one of the simpler function built into <code>convert</code> is
    <code>str2bool</code> that has many common use cases. Especially if working with APIs and integrations.
</p>
<code class="python">
    <pre>{`
...
<p>
    There are a few built-in functions for getting ranges and references to different time periods. These are
    specific to certain periods like “month”, “quarter”, “fiscal year”, etc.
</p>
<code class="python">
    <pre>{`
...
<h2>The others</h2>
<p>
    Check out some of the other modules that we didn’t cover. All of these are kept in the core repository
    under <code>odoo/tools</code>
    . There are tools for interaction with the local filesystem via <code>osutil.py</code> , pdf helpers via
    <code>pdf.py</code>
    , profiling and debugging tools under <code>profiler.py</code> or <code>debugger.py</code> , and some data
    structures built from scratch under <code>graph.py</code> and <code>lru.py</code> .
</p>
