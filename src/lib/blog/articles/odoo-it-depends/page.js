...
class PartnerGroup(models.Model):
    ...
    partner_ids = fields.One2many('res.partner', 'partner_group_id', 'Partners')
      ...
      <p>This is a class for defining groups of partners. It has a name for the group and a reference to all the partners in the group. Simple enough. No performance issues yet.</p>
...
class AccountInvoice(models.Model):
    ...
    def compute_is_late(self):
      ...
      <p>Now we’ve gotten a small system for defining late invoices and for checking if any given partner has a late invoice. There are two compute functions. One on each class and each has a single dependency. We can start mapping out what our dependency chain looks like.</p>
      <code className="python">
        <pre>{`
...
account.invoice:is_late    => account.invoice:date_invoice
      ...
      <p>Every time an invoice is updated on a partner it computes the partner’s has_late_bills field and every time an invoice’s date_invoice field is updated it recomputes the invoice’s is_late field.</p>
      <p>Again, this is pretty simple. Still no performance issues yet. Let’s update our partner group class to calculate if an entire group has any late bills.</p>
      <code className="python">
        <pre>{`
# -*- coding: utf-8 -*-
import logging
...
class PartnerGroup(models.Model):
    ...
    def compute_has_late_bills(self):
      ...
      <p>This includes a single mistake that can cause degraded performance and even unnecessary database queries (when using stored=True on compute functions) depending on how many concurrent users are accessing the system, the amount of invoices, the number of partners per partner group, etc.</p>
      <p>The groups has_late_bills depends on both the partners has_late_bills field and the partners invoices ids which is not necessary. Our dependency chain get’s slightly more complicated.</p>
      <code className="python">
        <pre>{`
...
res.partner.group:has_late_bills => partner_ids.invoice_ids
      ...
      <ul>
          ...
          <li>Compute res.partner.group:has_late_bills if one of the partners has a new has_late_bills value after recomputing it in the line above</li>
          <li>Compute res.partner.group:has_late_bills because the one of the partners in the group has a new invoice</li>
      </ul>
      <p>The last two actions are duplicates of each other. There is no reason why this needs to fire two times and can be resolved by updating our depends decorator.</p>
      <code className="python">
        <pre>{`
@api.depends(‘partner_ids.has_late_bills’)
        `}</pre>
      </code>
      <p>This may not be a huge problem in this specific example when updating a single invoice. It performs 4 operations instead of 3, but a 25% increase in computation starts to hurt at scale. Bulk editing 100 invoices turns into 400 operations instead of 300. 5 concurrent users bulk editing 100 invoices turns into 2000 operations instead of 1500.</p>

      {/* === */}

      <h2>In Conclusion</h2>
      <p>Think about this example within larger custom modules. This has 3 classes and 3 computed fields. In modules that have dozens or hundred of classes that use unnecessary dependencies on compute functions could severely hurt user experience.</p>
      <blockquote>Note: Watch out for stored, computed fields that cause both a computation and database write.</blockquote>
      <p>Luckily these are easy to track down! Log statements are your friend, and drawing simple dependency chain diagrams can allow you to cross out paths that already exists.</p>
