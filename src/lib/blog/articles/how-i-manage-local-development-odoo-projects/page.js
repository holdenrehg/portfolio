  ...
  <Layout>
    <Article meta={props.meta}>
      <ul className="table-of-contents">
        <li><strong><a href="#evolution-of-solutions">The evolution of solutions</a></strong>
            <ul>
      ...
      <p>
        <span className="first-letter">O</span>doo development has a unique set of problems
      ...
      <ul>
        <li>I only use docker for local databases now. This is because of the performance of running Odoo in a docker container. I'm a team of 1 now, so I don't need the standardization across multiple local environments and operating systems. I have the luxury of just dealing with my system.</li>
        <li>
          I switched to
          using <a href="https://github.com/pdm-project/pdm" target="_blank" rel="noreferrer"><code>pdm</code></a> or <a href="https://github.com/pypa/pipenv" target="_blank" rel="noreferrer"><code>pipenv</code></a>.
          Since I don't use docker for Odoo containers, I needed a way to manage virtual
          environments. I started with <code>pipenv</code> and liked it pretty well, but
          eventually switched to <code>pdm</code> because it entirely abstracts the virtual
          environment away from you. It works more like a traditional package
          manager (think <code>npm</code> for Node.js or <code>composer</code> for
          PHP).
        </li>
        <li>
          I use <a href="https://github.com/jacebrowning/gitman" target="_blank" rel="noreferrer"><code>gitman</code></a> to
          track Odoo source, enterprise addons, and any 3rd party git repo addons as
          dependencies. In the Odoo world, we for some reason don't typically
          make <code>pip</code> packages for addons. This leaves us to cloning down
          repositories and sticking them in our projects, or using submodules, or
          creating some other hacky solution. <code>gitman</code> has really been
          awesome for this. It let's me throw a <code>gitman.yaml</code> file in
          my repo, run <code>gitman update</code> and it downloads it all into a
          non-git tracked <code>vendor/</code> folder for me. I can even add
          dependencies with specific commit hashes to make upgrades more clear:
        </li>
      </ul>
...
/my_projects
    /my_cool_project
      ...
      <p>For where I'm at in my career with web development and Odoo development, this process actually works very nicely for me. It may not work well for everyone. You may be on a team of developers who does need that Docker layer of abstraction for the actual Odoo instance. Or you may have some specific outside dependencies on CI/CD tools that require you to setup projects in a certain way.</p>
      <p>Either way, I hope this gives you some helpful information or inspiration for your own projects at least.</p>
      <p>Best of luck coding.</p>
      <p>
          <mark className="bg-orange-700 text-orange-100 font-bold px-3 py-1">PS:</mark> Take a look at
          the <a href="https://github.com/rucksac/skeleton" target="_blank" rel="noreferrer">repository <code>README</code> for specific details</a> on
          using the skeleton project.</p>
    </Article>
  </Layout>
)
